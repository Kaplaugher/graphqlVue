        ...
        <v-card color="secondary" dark>
          <v-container>
            <v-form v-model="isFormValid" lazy-validation ref="form" @submit.prevent="handleSigninUser">
              ...
              <v-layout row>
                <v-flex xs12>
                  <v-btn :loading="loading" :disabled="!isFormValid || loading" color="accent" type="submit">Sign In
  ...
  data() {
    return {
      ...
      usernameRules: [
        ...
        // make sure username is less than 10 char
        username => username.length < 10 || 'Username must be less than 10 char'
      ],
      passwordRules: [
        password => !!password || 'Password Required',
        password => password.length >= 4 || 'Password must be at least 7 chars'
      ],
      isFormValid: true
    };
  },
